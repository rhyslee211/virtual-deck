...
import { FaTwitch, FaKeyboard } from 'react-icons/fa';

function Sidebar({onFormButtonClick, onEditButtonClick, isEditing, formState, onSettingsButtonClick, onHotkeyFormButtonClick, connectToOBS, obsConnected, connectToTwitch, isTwitchConnected}) {
    return (
        <div className="w-16 bg-slate-800 h-full max-h-full">
            <div className="flex flex-col items-center justify-between h-full">
                <div className="flex flex-col items-center">
                    ...
                    <button className={`w-12 h-12 mb-4 bg-indigo-400 rounded-full flex items-center justify-center hover:bg-indigo-500 ${formState === "hotkeyForm" ? 'border-2 border-white' : ''}`} onClick={onHotkeyFormButtonClick}><FaKeyboard size={24} /></button>
                </div>
                <div className="flex flex-col items-center">
